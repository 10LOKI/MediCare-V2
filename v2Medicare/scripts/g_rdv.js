...
function listRendezVous(){
    ...
    listRendezVous.innerHTML = arrayRdv.map(item => `
        <tr class="border-b-8 text-lg text-blue-950 bg-blue-600/50">
            ...
            <td class="flex flex-col md:flex-row items-center md:justify-evenly">
                ...
                <p id="status"
                class="flex items-center justify-center  w-20 text-sm font-extrabold duration-600 transition-all">${item.statusRdv}</p>

                <p onclick="refusRdv(${item.id})"
...
function acceptRdv(idState){
    ...
    if (confirm('Êtes-vous sûr de Accepté ce rendez-vous ?')) {
        acceptAppointments = arrayRdv.map(apt =>
