    ...
    if (!list_Rendez_Vous) return;
        ...
        <tr class="border-b shadow-md text-md md:text-xl lg:text-2xl text-blue-950 bg-blue-300">
            ...
            <td class="flex flex-col md:flex-row bg-amber-400 items-center md:justify-evenly">
                <div id="accept"
                class="
                flex items-center justify-center cursor-pointer w-8 h-14">
                <div class="hover:-rotate-180 w-full h-8 rounded-full bg-center bg-no-repeat bg-[url('../../v1Medicare/images/verifie.png')] duration-500 transition-all"></div>
                </div>

                <div id="status"
                class="flex items-center justify-center  w-20 text-sm font-bold hover:font-extrabold duration-600 transition-all">${item.status}</div>
...
function refus(id_State){
    if (confirm('Êtes-vous sûr de refuser ce rendez-vous ?')) {
        appointments = arrayRdv.map(apt =>
            apt.id_State === id ? { ...apt, status: 'refusé' } : apt
        );
